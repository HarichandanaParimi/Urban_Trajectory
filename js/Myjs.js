...
map.on('draw:created', function (e) {
	...
	if (type === 'rectangle') {
		...
		console.log(result[0]);		// Trip Info: avspeed, distance, duration, endtime, maxspeed, minspeed, starttime, streetnames, taxiid, tripid
    DrawRS(result);
		// WordCloud(result);
    console.log("avgspeed")
    for (let key in result){
   if(result.hasOwnProperty(key)){
     console.log(result[key].avspeed)
   }
}
  console.log("distance")
  for (let key in result){
 if(result.hasOwnProperty(key)){
   console.log(result[key].distance)
 }
}
...
for (let key in result){
if(result.hasOwnProperty(key)){
 console.log(result[key].maxspeed)
}
}
